...
import type { PoolTicketItem, SupportTicket, TicketStatus, UserAccount } from "@/types/api";

type SortKey = "id" | "status" | "created_at" | "assignee";
type SortDir = "asc" | "desc";

const FILTER_OPTIONS: { value: TicketStatus | "all"; label: string }[] = [
  { value: "all", label: "Все" },
  { value: "in_queue", label: "В очереди" },
  { value: "important", label: "Важное" },
  { value: "in_progress", label: "В работе" },
  { value: "transferred_to_engineers", label: "Передан инженерам" },
];
...
export default function AgentPoolPage() {
  ...
  const loadPool = useCallback(async () => {
    ...
    try {
      ...
      const payload = await fetchTicketPool(filter);
  ...
  return (
    <div className="agent-page">
      <div className="agent-top-row">
        <div className="agent-nav-card">
          ...
          <div className="agent-nav-list">
            <button type="button" className="agent-nav-item is-active">
              Пул тикетов
            </button>
        ...
        <div className="agent-profile-card">
          <div className="agent-profile-text">
            ...
            <div className="agent-profile-role">{formatAgentBadge(user.user_account_id)}</div>
          </div>
          <button
            type="button"
      ...
      <section className="agent-main-card">
        ...
        {filterOpen && (
          <div className="agent-filter-menu">
            {FILTER_OPTIONS.map((option) => (
              <button
                key={option.value}
                type="button"
                className={`agent-filter-chip${statusFilter === option.value ? " is-active" : ""}`}
        ...
        <p className="agent-claim-hint">
          Общий пул: нажмите на свободный тикет, чтобы взять его в работу.
        </p>
          ...
          {!loading && !error && sortedItems.length > 0 && (
            <table className="agent-table">
              <thead>
                <tr>
                  <th className="col-id">
                    <button type="button" className="agent-nav-item" onClick={() => toggleSort("id")}>
                      ID тикета
                    </button>
                  </th>
                  <th className="col-status">
                    <button type="button" className="agent-nav-item" onClick={() => toggleSort("status")}>
                      Статус
                    </button>
                  </th>
